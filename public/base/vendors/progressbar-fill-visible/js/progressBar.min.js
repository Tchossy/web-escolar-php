...
!(function (t, n) {
  ...
  function o(o, e = {}) {
    ...
    console.log(y),
      r.el.addClass('progress-' + g),
      (r.bar = r.el.find('.ab-progress-bar')),
      (r.title = r.el.find('.progress-title')),
      (r.indecator = r.el.find('.progress-indecator')),
      (r.numWrap = r.indecator.find('.progress-indecator-inner')),
      (r.number = r.indecator.find('.percent'))
    ;(r.started = !1),
      (r.stopped = null),
      (r.animator = null),
      (r.progress = 0),
      (r.start = function () {
        r.started ||
          ((r.animator = t({ Progress: r.progress }).animate(
            { Progress: s },
            {
              duration: h,
              easing: m,
              ...
              progress: function () {
                ...
                ;(r.progress = n.Progress),
                  r.number.text(
                    (E ? '%#' : '#%').replace('#', Math.ceil(n.Progress))
                  ),
                  r.bar.css({ width: n.Progress + '%' }),
                  r.numWrap.css({ left: n.Progress + '%' }),
      ...
      !r.title.length &&
        l &&
        ((r.title = t(c).addClass('progress-title')),
        r.title.text(u),
        r.el.html(r.title)),
      r.indecator.length ||
        ((r.indecator = t('<div/>').addClass('progress-indecator')),
        r.title.after(r.indecator)),
      r.number.length ||
        ((r.number = t('<span class="percent"></span>')),
        r.number.appendTo(r.indecator),
        r.number.wrap('<div class="progress-indecator-inner"/>'),
        (r.numWrap = r.number.parent()),
        'tooltip' === g && r.number.after('<span class="down-arrow"/>')),
      r.bar.length ||
        ((r.bar = t('<div/>').addClass('ab-progress-bar')),
        r.indecator.after(r.bar),
        r.bar.wrap('<div class="progress-bar-wrap"/>')),
      E && r.el.addClass('progress-rtl'),
      y &&
        (C && n.Waypoint
          ? new Waypoint({
              element: r.el[0],
